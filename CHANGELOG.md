...
- 需要 Bot commit `0547b2389a27959954863af32a44e6e7f1971ab9` 及以上；请勿只升控制台
- **任务编排**：
  - 新增记忆与后台任务：群事件记忆、群友偏好、IP 知识、记忆图谱、会话摘要、好感度判定、语义风格标注
...
- 需要 Bot commit `0547b2389a27959954863af32a44e6e7f1971ab9` 及以上；请勿只升控制台
- **大模型配置**：
  - 提供方预设新增 Ollama 原生请求方式，思考模型直连对话接口
  - 定价规则支持多时段区间，可配置跨零点时段
  - 注册模型配置默认折叠显示
  - 统一大模型配置导航文案
- **媒体配置**：
  - 媒体配置独立为单独页面
...
- 需要 Bot commit `7f7073cde3562c030f1ae213e169f6d53c935574` 及以上；请勿只升控制台
- **语义风格治理**：
...
- 需要 Bot commit `568fe430848ed063cc1ab8841c1f12ce2530936c` 及以上；请勿只升控制台
- **黑名单审计**：
  - 数据库页「存储」新增「黑名单审计」只读入口
  - 支持查看全局用户、群、群内用户的拉黑 / 解禁历史
  - 展示目标、动作、操作者、原因和时间，并支持分页与完整行详情
  - 好友配置列表补充最近一次拉黑 / 解禁操作及操作者搜索
- **插件目录**：
  - 展示社区插件缺失的运行依赖
- **媒体服务**：
  - 安装区补充详细安装文档链接
...
* feat(database): 展示黑名单审计历史
* feat(webui): 插件目录展示社区插件缺失依赖
...
- 需要 Bot commit `aa1f1092` 及以上；请勿只升控制台
- **AI 配置**：
  - 新增「预算」分段，集中管理各任务每日调用/输入上限与上下文预算
  - 预算字段帮助标注次数与 token 单位
...
- 需要 Bot commit `34a0d840` 及以上；请勿只升控制台
- **AI 观测**：
  - 好感度判定任务补中文名，观测更易读
  - 「醉聊」统一为「酒后聊天」
- **AI 治理**：
...
- 需要 Bot commit `1e713e82` 及以上；请勿只升控制台
- **AI 治理**：
  - 群风格与语义面板合并，治理页信息分层更聚焦，直接查看某群的语义样本与回复形态
  - 展示群语义实际样本：列出该群整理出的前句→接话、接话复盘与行为策略
  - 任务名中文化，观测与统计更易读
- **AI 观测**：
  - 统计页新增「导出」：按当前日期区间导出计费统计 CSV，含区间汇总、每日明细与 Token / 画画的 Provider、模型、任务明细，Excel 可直接打开
...
- 需要 Bot commit `1e198188` 及以上；请勿只升控制台
- **Bot 更新**：
  - Bot 更新面板列出的提交条数可配置，并补上展开三角与全页风格统一
- **任务编排**：
  - 模型下拉列宽自适应，长模型名不再溢出；「视觉选图」与表情图分节说明「聊天看图」均复用该视觉模型
  - Provider 默认设置文案优化
- **AI 观测**：
  - 回复路径剔除语料选句，观测更准
- **配置**：
  - base_url 提示默认补 `/v1`，支持 `/v4` 等版本段
- **模型兼容**：
...
- 需要 Bot commit `1e198188` 及以上；请勿只升控制台
- **AI 治理**：
  - 移除反哺写回入库面板与相关类型，治理页更聚焦；已接入的写回调整回归纠错加权
- **仪表盘**：
...
- 需要 Bot ≥ 4.3.8；请勿只升控制台。
- **AI 治理**：
  - 重构为「Prompt 流水线」页，回复生成按身份 / 时机 / 形态 / 措辞 / 发送五阶段可视化；观测下的人物 / 记忆并入治理页
  - 旧「牛格」页与 AI 观测原入口重定向到治理页对应标签
- **基础提示词**：
...
- 需要 Bot ≥ 4.3.6；请勿只升控制台。
- **AI 观测 · 人物**：
  - 关系笔记新增按内容、来源、QQ 号搜索
  - 可在控制台直接手动修改关系备注文本
  - 好感度等级与数值可手动调整，越界输入会提示范围
- **界面**：
  - 下拉选择弹出不再向上越出屏幕，手机端也能看到完整选项
  - 手机端输入控件字号提升到 16px，聚焦输入不再触发页面自动放大
  - 更新页 Commit 历史默认折叠，只显示最新一条，点开可看全部
- **AI 配置**：
...
- 需要 Bot ≥ 4.3.5；请勿只升控制台。
- **AI 配置**：
  - 恢复「会话内核」配置段，关系备注（含好感度等级与手动调整）、会话状态与记忆条目可在控制台直接查看。
- **界面**：
  - 修复密钥类字段的标签在窄标签行时换行的问题。
...
- 需要 Bot ≥ 4.3.2；请勿只升控制台。
- **插件管理**：
  - 商店卡片更新按钮统一显示；插件管理卡片新增三点菜单与禁用按钮。
  - 移除插件管理页的部署模式顶部提示。
...
- 需要 Bot ≥ 4.2.5；请勿只升控制台。
- **AI 统计**：
  - 回复路径按触发方式拆分展示，接话·语料选句与 @ 直出、别名感知、主动发言、续聊区分显示

### 提交明细

### Added

* feat(ai): 回复路径展示触发方式与接话拆分
...
- 仍需 Bot ≥ 4.2.1；请勿只升控制台。
- **图表**：
  - 点击坐标可固定信息面板，再次点击或点击图表外取消
- **运行日志**：
  - 固定面板新增复制整行按钮
  - 报错展开正文自适应视口并内部滚动
- **偏好设置**：
...
- 需要 Bot ≥ 4.2.1；请勿只升控制台。
- **报错页**：
  - 改为独立可展开小卡片，去掉嵌套面板内滚动，查看报错更直观
- **插件**：
  - 卸载统一收敛到管理页，内置插件提示不可卸载，卸载需输入插件名确认
  - 管理页与商店卡片按钮配色、版本标签不换行与窄屏布局优化
- **统计页**：
  - 新增表情标签区与表情视觉空状态
- **日志**：
  - 行首业务前缀高亮支持中文空格、连字符与纯字母前缀
...
- 需要 Bot ≥ 4.2.0；请勿只升控制台。
- **数据库生命周期**：
  - 物理对象列支持自动换行，长表名、索引名不再截断或撑宽表格，窄屏查看更完整
- **侧栏与日志**：
  - 重启会优先使用轻量重启能力，控制台不必等待完整系统操作返回
  - 运行日志级别徽章对齐与扫描更清晰
- **插件**：
  - 来源筛选、卡片标签和版本展示保持一致，官方、社区与本地插件更容易区分
- **界面**：
  - 实例卡为昵称预留稳定宽度，短名称不会挤压状态信息
  - 弹窗关闭按钮统一为标题栏操作，减少重复按钮
...
- 需要 Bot ≥ 4.2.0；请勿只升控制台。
- **数据库**：
  - 新增生命周期管理页：查看各数据对象的规模与保留策略，预览清理影响后确认执行，并跟踪运行结果
- **AI 观测**：
  - 新增群表达与表情标签维护，可直接查看和维护表情语义标签与群表达画像
- **更新**：
  - 开放 Docker Release 与协议镜像更新，并统一汇总 Bot、AI Runtime 与协议端更新状态
- **AI 配置**：
  - 新增代理歌曲缓存生命周期设置
- **插件**：
  - 卡统一来源标签、描边与状态样式，深色模式可读性提升
- **修复**：
  - 协议端未加载插件时的误报挂载异常、插件治理配置按字段保存、图表悬停提示跟随等问题
...
- 仍需 Bot ≥ 4.1.32；请勿只升控制台。
- **界面**：
  - 优化控制台面板的边界、网格与加载态，信息层级更清晰
- **AI 观测**：
  - 补充费用、用量与调用详情展示，并统一进度与占比视图
- **模型与日志**：
  - 模型价格条件支持连续输入小数
  - 运行日志默认查看主进程来源
...
- **入站调度**：
  - 面板补齐学习流水线、近期历史、执行容量与预算高压趋势，便于判断高峰降级和排队状态
- **智能对话**：
  - 支持视觉选图任务编排与复读语义风格配置，常用模型配置更紧凑、密钥识别更清晰
- **窄屏**：
  - 优化顶部品牌与工具栏操作区显示
...
- **协议**：
  - 创建账号会读取并显示协议管理的默认后端，未手动修改时与私聊“创建牛牛”保持一致
...
- **入站调度**：
  - 面板展示队列、车道与 P95 分级，高峰状态更容易判断
- **日志**：
  - 支持查看消息主进程、work aux 与 embed aux 的独立来源
- **协议**：
  - SnowLuma Runtime 支持选择独立镜像，协议运行环境切换更清晰
- **插件商店**：
  - 进入后会清除更新提醒，避免已查看内容持续显示绿点
