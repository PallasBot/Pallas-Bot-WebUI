...
- 图表点击坐标可固定信息面板，再次点击或点击图表外取消，长图查看更省心。
- 运行日志固定面板新增复制整行按钮；报错展开正文自适应视口并内部滚动。
- 偏好页主题色选择不再受黑白预设限制，始终可调；跟随系统深浅色会实时切换。
- 深色模式整体校准：导航选中、开关、上新点与链接悬停对比更清晰；幽灵按钮不再出现悬停背景块。
...
- 报错页改为独立可展开小卡片，去掉嵌套面板内滚动，查看报错更直观。
- 插件卸载统一收敛到管理页：内置插件提示不可卸载，卸载需输入插件名确认。
...
- 数据库生命周期页的物理对象列支持自动换行，长表名、索引名不再截断或撑宽表格，窄屏查看更完整。
- 侧栏重启会优先使用轻量重启能力，控制台不必等待完整系统操作返回；运行日志级别徽章的对齐与扫描更清晰。
- 插件来源筛选、卡片标签和版本展示保持一致，官方、社区与本地插件更容易区分。
- 实例卡为昵称预留稳定宽度，短名称不会挤压状态信息；弹窗关闭按钮统一为标题栏操作，减少重复按钮。
...
- 需要 Bot ≥ 4.2.0；请勿只升控制台。
- 新增数据库生命周期管理页：查看各数据对象的规模与保留策略，预览清理影响后确认执行，并跟踪运行结果。
- AI 观测新增群表达与表情标签维护，可直接查看和维护表情语义标签与群表达画像。
- 更新页开放 Docker Release 与协议镜像更新，并统一汇总 Bot、AI Runtime 与协议端更新状态。
- AI 配置新增代理歌曲缓存生命周期设置。
